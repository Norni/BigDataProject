...
select
    ...
    if(old.mid_id is null,'$now_date',old.login_date_first),
    if(new.mid_id is not null,'$now_date',old.login_date_last),
...
select
    ...
    if(old.login_date_first is null and new.login_count>0,'$now_date',old.login_date_first),
    if(new.login_count>0,'$now_date',old.login_date_last),
    nvl(old.login_count,0)+if(new.login_count>0,1,0),
    nvl(new.login_last_30d_count,0),
    if(old.order_date_first is null and new.order_count>0,'$now_date',old.order_date_first),
    if(new.order_count>0,'$now_date',old.order_date_last),
    nvl(old.order_count,0)+nvl(new.order_count,0),
    nvl(old.order_amount,0)+nvl(new.order_amount,0),
    nvl(new.order_last_30d_count,0),
    nvl(new.order_last_30d_amount,0),
    if(old.payment_date_first is null and new.payment_count>0,'$now_date',old.payment_date_first),
    if(new.payment_count>0,'$now_date',old.payment_date_last),
...
(
    select
        user_id,
        sum(if(dt='$now_date',login_count,0)) login_count,
        sum(if(dt='$now_date',order_count,0)) order_count,
        sum(if(dt='$now_date',order_amount,0)) order_amount,
        sum(if(dt='$now_date',payment_count,0)) payment_count,
        sum(if(dt='$now_date',payment_amount,0)) payment_amount,
...
(
    select
        sku_id,
        sum(if(dt='$now_date', order_count,0 )) order_count,
        sum(if(dt='$now_date',order_num ,0 ))  order_num,
        sum(if(dt='$now_date',order_amount,0 )) order_amount ,
        sum(if(dt='$now_date',payment_count,0 )) payment_count,
        sum(if(dt='$now_date',payment_num,0 )) payment_num,
        sum(if(dt='$now_date',payment_amount,0 )) payment_amount,
        sum(if(dt='$now_date',refund_count,0 )) refund_count,
        sum(if(dt='$now_date',refund_num,0 )) refund_num,
        sum(if(dt='$now_date',refund_amount,0 )) refund_amount,
        sum(if(dt='$now_date',cart_count,0 )) cart_count,
        sum(if(dt='$now_date',favor_count,0 )) favor_count,
        sum(if(dt='$now_date',appraise_good_count,0 )) appraise_good_count,
        sum(if(dt='$now_date',appraise_mid_count,0 ) ) appraise_mid_count ,
        sum(if(dt='$now_date',appraise_bad_count,0 )) appraise_bad_count,
        sum(if(dt='$now_date',appraise_default_count,0 )) appraise_default_count,
...
left join
(select * from dwd_dim_sku_info where dt='$now_date') sku_info
...
(
    select
        ...
        region_id,
        region_name,
        sum(if(dt='$now_date',login_count,0)) login_day_count,
        sum(if(dt='$now_date',order_count,0)) order_day_count,
        sum(if(dt='$now_date',order_amount,0.0)) order_day_amount,
        sum(if(dt='$now_date',payment_count,0)) payment_day_count,
        sum(if(dt='$now_date',payment_amount,0.0)) payment_day_amount,
